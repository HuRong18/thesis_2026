...
取辅助矩阵$\mathbf{E}=\mathbf{I}_K$，即$K\times K$单位矩阵，这意味着解码器的目标是恢复$\hat{\mathbf{g}}_{\mathbf{I}_K}^{(t)}=[\tilde{\mathbf{g}}_{\pi(1)}^{(t)},\cdots,\tilde{\mathbf{g}}_{\pi(K)}^{(t)}]$，即完整恢复所有参与传输节点的编码梯度。
因此，$L_{\mathbf{E}}=K$。由于需要恢复所有信息，干扰矩阵$\mathbf{C}$必须为空，即$L_{\mathbf{C}}=0$，否则无法区分具体信源。集合$\mathcal{S}=[1:K]$，代入定理\ref{bcst}中的可达速率区域定义可得
\begin{equation}
...
取辅助矩阵$\mathbf{E}=\mathbf{b}$，即解码系数行向量，大小为$1\times K$，这意味着解码器的目标直接为$\hat{\mathbf{g}}_{\mathbf{b}}^{(t)}=\sum_{k\in\mathcal{K}}b_k\tilde{\mathbf{g}}_k^{(t)}=\mathbf{g}^{(t)}$。
...
\end{equation}
由于$\mathbf{E}=\mathbf{b}$是一个非零行向量，要使式(\ref{full rank})满秩，每一个节点都必须包含在$\mathcal{T}$中或被$\mathbf{I}_K$覆盖。这意味着对于每一个独立的节点$k\in\mathcal{K}$（令$\mathcal{T}=\{k\}$），为满足秩约束，不等式退化为
\begin{align}
	\tilde{R}_{c,k}>H(\hat{\mathbf{g}}_{\mathbf{b}}^{(t)})=H(\mathbf{g}^{(t)}),\quad\forall k\in \mathcal{K}.
\end{align}
其和速率下界为
\begin{align}
	\sum_{k\in\mathcal{K}}\tilde{R}_{c,k}\geq K\cdot H(\mathbf{g}^{(t)}),
\end{align}
...
\begin{align}
	\tilde{R}_{linear}^{sum}=\sum_{k\in\mathcal{K}}H(\mathbf{g}^{(t)})=K\cdot H(\mathbf{g}^{(t)}).
\end{align}
当寻求$\tilde{R}_{linear}^{sum}<\tilde{R}_{sw}^{sum}$的条件，即
\begin{align}
	K\cdot H(\mathbf{g}^{(t)})<H\left(\{\tilde{\mathbf{g}}_k^{(t)}\}_{k\in\mathcal{K}}\right)
\end{align}
