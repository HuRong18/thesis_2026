...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.77\textwidth]{figs/fig1.eps}
	\caption{编码计算示例}
...
该式表明，梯度编码的本质是设计一个编码矩阵 $\mathbf{B}$ 必须满足，对于任意具有 $N-S$ 个非零元素的解码向量 $\mathbf{A}$，均存在解使得 $\mathbf{A}\mathbf{B} = \mathbf{1}_{1 \times K}$（全1向量）。
 ...
 \begin{figure}[htbp]
	\centering
	\includegraphics[width=0.77\textwidth]{gradient_coding.pdf}
	\caption{梯度编码计算}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.7\textwidth]{figs/fig2.eps}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.5\textwidth]{figs/fig3.eps}
