...
分布式边缘计算通过将计算任务划分为多个子任务并分发至多个边缘节点并行处理来降低时延。然而，由于边缘节点可能因系统故障、资源竞争等随机因素导致计算延迟或失败，中心节点需要等待所有边缘节点完成计算并返回结果后才能恢复原始计算结果，这会导致整体计算时延的增加。编码边缘计算通过引入纠删码理论向计算任务中注入冗余，可以有效缓解迟滞效应带来的影响。具体而言，在编码边缘计算场景中，中心节点首先将原始计算任务划分为多个子任务，并通过纠删码生成多个编码子任务；随后将编码子任务分发给边缘节点进行并行计算；当任意恢复阈值个或更多节点完成计算后，中心节点即可从中恢复原始计算结果，从而避免等待迟滞节点。
...
编码端在观测到信源序列 $\mathbf{Y}_1$ 后，并不直接传输原始数据，而是计算并发送其对应的伴随式 $\mathbf{s} = \mathbf{H}\mathbf{Y}_1^T$。由于伴随式的长度仅为 $n-k$，该方案将传输速率压缩至 $(n-k)/n$。在解码端，伴随式 $\mathbf{s}$ 唯一确定了 $\mathbf{Y}_1$ 所在的陪集。解码器将边信息 $\mathbf{Y}_2$ 视为 $\mathbf{Y}_1$ 经过虚拟相关信道干扰后的观测值，并在指定的陪集内搜索与 $\mathbf{Y}_2$ 具有最大后验概率的序列 $\mathbf{\hat{Y}}_1$。这一过程将分布式信源编码问题转化为了经典的信道纠错解码问题。
...
\begin{figure}[htbp]
	...
	\label{nlc}
\end{figure}
为了更清晰地阐述定理\ref{<theorem-chapter-2>}的可达速率区域，图\ref{nlc}给出了包含两个节点的系统、无损恢复一个线性组合的示例速率区域。其中，蓝色实线区域是SW可达速率区域，红色点线是Körner-Marton 可达速率区域，黑色虚线区域表示使用嵌套线性码独立恢复两个信源再进行线性计算的可达速率区域。定理\ref{<theorem-chapter-2>}可以实现Körner-Marton区域和黑色虚线区域的并集，包含了SW区域，实现了比SW压缩更优的效果。
...
在编码计算场景中，当边缘节点完成计算任务后，需要通过下行无线信道将计算结果回传给中心节点进行恢复。为提升频谱效率，下行传输考虑采用非正交多址接入（Non-Orthogonal Multiple Access，NOMA）技术，其他多址接入技术（如速率分割多址接入\upcite{9831440}）也同样适用。
