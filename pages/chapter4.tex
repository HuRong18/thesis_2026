...
本章后续内容的安排如下：第\ref{4.2}节介绍了面向线性恢复的梯度编码计算下行传输模型；第\ref{4.3}节详细阐述了所提的面向线性恢复的DSC辅助编码计算下行传输机制；第\ref{4.4}节针对所提机制下的最小化传输时延问题进行求解，设计相应的优化算法；第\ref{4.5}节通过数值仿真实验结果对所提机制的效果进行验证；第\ref{4.6}节对本章内容进行了总结。本章重要符号参见表\ref{<chapter-four-table-symbols>}。
\begin{table}
	%\small
	\centering
	\caption{本章常用符号}
...
\section{面向线性恢复的DSC辅助编码计算结果下行传输机制}\label{4.3}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width = 0.65 \textwidth]{real_channel_gain.eps}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width = 0.65 \textwidth]{real_transmit_power.eps}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width = 0.65 \textwidth]{equal_channel_gain.eps}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width = 0.65 \textwidth]{equal_transmit_power.eps}
