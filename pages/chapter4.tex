...
\section{基于 CCCP 的时延优化算法}\label{4.4}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width = 0.7 \textwidth]{figs/real_channel_gain_Chinese.eps}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width = 0.63 \textwidth]{figs/N_increase_Chinese.eps}
	\caption{不同参与计算的边缘节点数目$N$下的下行传输时延$T_d$比较。}
	\label{fig:4.6}
\end{figure}

图~\ref{fig:4.6}和图~\ref{fig:4.7}分别展示了不同参与计算的边缘节点数目$N$和不同参与下行传输的边缘节点数目$K$下的传输时延$T_d$比较，其中假设数据集大小$v=10000$。从图~\ref{fig:4.6}可以看出，当参与下行传输的边缘节点数目$K$固定时，参与计算的边缘节点数目越多，所提线性恢复编码方案相比于基线方案的性能增益越明显。例如，当$N=5$时，所提线性恢复方案的下行传输时延$T_d$为$202.8$（ms），基线方案1（理论SW编码）对应的下行传输时延为$343.2$（ms），基线方案2对应的下行传输时延为$422.1$（ms），所提线性恢复编码方案相比于基线方案分别提升了$41\%$和$52\%$。当$N=10$时，所提线性恢复方案的下行传输时延$T_d$为$392.1$（ms），基线方案1（理论SW编码）对应的下行传输时延为$1224.2$（ms），基线方案2对应的下行传输时延为$1224.2$（ms），所提线性恢复编码方案相比于基线方案提升了$68\%$。这是由于当计算的边缘节点数目$N$增加，恢复阈值$K$不变时，即迟滞节点数目变多，每个节点需要计算的数据量增加且引入的编码系数也增多，从而使得单个节点所包含的信息熵增加。这使得节点间在进行信源压缩时，受到了约束为单个节点的信息熵约束更明显，从而压缩的性能受限。而所提线性恢复编码方案只增加了数据集的信息熵，没有引入更多的编码系数，影响较低。此外可以观察到，当$N$增加时，基线方案1和基线方案2的时延相等，可以验证上述的原因，即信源压缩受到了单个节点的最小编码速率约束。
...
\begin{figure}[htbp]
	...
	\label{fig:4.7}
\end{figure}
类似地，从图~\ref{fig:4.7}中可以观察到，当参与下行传输的节点数目越少时，所提线性恢复编码方案相比于基线方案的性能增益更明显。例如，当$K=3$时，所提线性恢复方案的下行传输时延$T_d$为$329.8$（ms），基线方案1（理论SW编码）对应的下行传输时延为$1224.2$（ms），基线方案2对应的下行传输时延为$1224.2$（ms），所提线性恢复编码方案相比于基线方案提升了$68\%$。当$K=7$时，所提线性恢复方案的下行传输时延$T_d$为$164.9$（ms），基线方案1（理论SW编码）对应的下行传输时延为$241.2$（ms），基线方案2对应的下行传输时延为$357.1$（ms），所提线性恢复编码方案相比于基线方案分别提升了$32\%$和$54\%$。这是由于，当参与下行传输的节点数目降低时，即迟滞节点数目增加，同样的会出现由于编码系数的引入导致单个节点的信息熵增加，受到单个节点信息速率约束的影响，DSC效果变差，从而增加下行传输时延。
